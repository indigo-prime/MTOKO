...
export default function CombinedSearchFilter3({ onFilterChange }: CombinedSearchFilterProps) {
  ...
  useEffect(() => {
    async function fetchCategories() {
      try {
        ...
        const fetchedCategories: Category[] = (data ?? []).map((item: SubCategoryRow) => ({
          name: item.name,
          image: item.imageUrl,
        }));
  ...
  useEffect(() => {
    onFilterChange({ searchTerm, selectedMood, selectedCategories, priceRange });
  ...
  return (
    <div className="space-y-6">
      {/* Category scroller */}
      <section className="mt-[72px] px-[20px] sm:px-[10px] md:px-[15px]">
