...
interface CombinedSearchFilterProps {
    ...
    /** NEW: pass a MainCategoryEnum value to limit subcategories to that main category */
    mainCategoryEnum?: string; // e.g. "FOOD_PACK"
}
...
export default function CombinedSearchFilter3({
                                                  onFilterChange,
                                                  mainCategoryEnum,
    ...
    // Fetch only subcategories for the provided main category (if given)
    useEffect(() => {
        async function fetchCategories() {
            try {
                ...
                if (mainCategoryEnum) {
                    // 1) Find the MainCategory row by enum name to get its id
                    ...
                    // 2) Fetch subcategories belonging to that mainCategoryId
                    ...
                    const fetched: Category[] = (data ?? []).map((item: any) => ({
                        name: item.name,
                    ...
                    // Fallback: old behavior (all subcategories)
                    const { data, error } = await supabase
                        .from("SubCategory")
                        .select("name,imageUrl")
                        .order("name", { ascending: true });
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Filters card */}
            <Card className="mb-8 shadow-soft max-w-[935px] mx-auto mt-[72px]">
                <CardContent className="p-6 space-y-6">
                    <div className="flex items-center gap-2 mb-2">
                        <Filter className="w-5 h-5 text-sunset-orange" />
                        <h3 className="text-lg font-semibold text-ocean-blue">Filter your Spot</h3>
                    </div>
            ...
            {/* Category scroller */}
            <section className="mt-[20px] px-[20px] sm:px-[8px] md:px-[13px]">
